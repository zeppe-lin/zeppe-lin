...
you to specify custom location via B<-H> option.  See
L<mkinitramfs(8)|https://github.com/zeppe-lin/zeppe-lin/blob/master/system/mkinitramfs/mkinitramfs.8.pod>
...
If message was not specified, then mkinitramfs will print default error
...
already.  Also no need to prepend path of mkinitramfs working directory.
...
module.  Create F</etc/mkinitramfs/hooks/ext4> directory and copy below
scripts with appropriate names to that directory.  After that, prepend
B<ext4> to B<hooks> option in mkinitramfs' config
