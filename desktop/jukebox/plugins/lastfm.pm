...
sub Stop {
    ...
    ::Update_QueueActionList();
    $queuewaiting->abort if $queuewaiting;
    $queuewaiting = undef;
}
...
sub new {
    ...
    $stateventbox->signal_connect(
        button_press_event => sub {
            my ($stateventbox, $event) = @_;
            return 0 unless $event->button == 3;
            my $ID = ::GetSelID($stateventbox);
            ::ArtistContextMenu(Songs::Get_gid($ID, 'artists'),
                {ID => $ID, self => $stateventbox, mode => 'S'})
              if defined $ID;
            return 1;
        }
    );    # FIXME: do a proper cm
    ...
    my $artistpic_create = sub {
        ...
        my $child = Layout::NewWidget(
            "ArtistPic",
            {   forceratio => 1,
                minsize    => $::Options{OPT . 'ArtistPicSize'},
                click1     => \&apiczoom,
                xalign     => 0,
                group      => $group,
    ...
    my $store = Gtk2::ListStore->new(
        'Glib::String', 'Glib::Double', 'Glib::String', 'Glib::UInt',
        'Glib::String'
    );
    ...
    $tc_similar->set_cell_data_func(
        $renderer,
        sub {
            my ($column, $cell, $model, $iter, $func_data) = @_;
            my $rating = $model->get($iter, 1);
            $cell->set(text => sprintf '%.1f', $rating);
        },
        undef
    );    # limit similarity rating to one decimal
...
sub toggled_cb {
    ...
    if ($togglebutton->get_active) {
        ...
        $self->SongChanged;
        $textview->set_tooltip_text($self->{site})
          if $self->{site} ne "events";
    }
}
...
#);
    ...
    $frame_similar->add(
        ::Vpack(
            ...
            $similar_local, $similar_exclude_seed
        )
    );
...
sub filename_preview {
    ...
    my $t =
      ::pathfilefromformat($::SongID, $::Options{OPT . 'PathFile'}, undef, 1);
...
sub apiczoom {
    ...
    my $menu = Gtk2::Menu->new;
    $menu->modify_bg('normal', Gtk2::Gdk::Color->parse('black'))
      ;    # black bg for the artistpic-popup
    ...
    my $label  = Gtk2::Label
      ->new;    # use a label instead of a normal menu-item for formatted text
    $item->modify_bg('selected', Gtk2::Gdk::Color->parse('black'));
    $label->modify_fg($_, Gtk2::Gdk::Color->parse('white'))
      for qw/normal prelight/;
...
sub button_release_cb {
    ...
    my ($x, $y) =
      $textview->window_to_buffer_coords('widget', $event->x, $event->y);
...
sub ArtistChanged {
    ...
    if (!$self->{url} or ($url ne $self->{url}) or $force) {
        ...
        {    # check for local biography file before loading the page
            unless ($force) {
                my $file =
                  ::pathfilefromformat(::GetSelID($self),
...
sub GetUrl {
    ...
    $url =~ s/%a/$artist/;
    my $limit = "";
    if ($::Options{OPT . 'SimilarLimit'} != "0") {
        $limit = $::Options{OPT . 'SimilarLimit'};
    }
    $url =~ s/%l/$limit/;
    return $url;
}
...
sub loaded {
    ...
    $encoding = $1
      if $data
      =~ m#<meta *http-equiv="Content-Type" *content="text/html; charset=([\w-]+)"#;
    $encoding = 'cp1252'
      if $encoding
      && $encoding eq 'iso-8859-1'
      ; #microsoft use the superset cp1252 of iso-8859-1 but says it's iso-8859-1
    ...
    my $tag_extra = $buffer->create_tag(
        undef,
        foreground_gdk => $self->style->text_aa("normal"),
        justification  => 'left'
    );
...
=pod
        ...
        for my $event (split /<\/event>/, $data) {
            my %event;
            $event{$1} = ::decode_html($2)
              while $event =~ m#<(\w+)>([^<]*)</\1>#g
              ; # FIXME: add workaround for description (which includes <tags>)
            next
              unless $event{id}
              ;    # otherwise the last </events> is also treated like an event
            $event{startDate} = substr($event{startDate}, 0, -9)
              ;    # cut the useless time (hh:mm:ss) from the date
            ...
            $buffer->apply_tag(
                $href,
                $buffer->get_iter_at_offset($offset1),
                $buffer->get_iter_at_offset($offset2)
            );
            $buffer->insert_with_tags($iter, "\n" . $rest, $tag_extra)
              if $rest;
        }
=cut
        ...
        for my $s_artist (split /<\/artist>/, $data) {
            my %s_artist;
            $s_artist{$1} = ::decode_html($2)
              while $s_artist =~ m#<(\w+)>([^<]*)</\1>#g;
            next
              unless $s_artist{name}
              ;   # otherwise the last </artist> is also treated like an artist
...
sub PopulateQueue {
    $queuewaiting = undef;
    if ($nowplayingaID != Songs::Get_gid($::SongID, 'artist')) {
        QAutofillSimilarArtists;
        return;
    }
    my $data = $_[0];

    return
      unless $::QueueAction eq
      'autofill-similar-artists';    # re-check queueaction and
    ...
    push(@artist_gids, Songs::Get_gid($::SongID, 'artist'))
      unless $::Options{OPT . 'SimilarExcludeSeed'}
      ;    # add currently playing artist as well

    my $filter = Filter->newadd(0, map Songs::MakeFilterFromGID("artist", $_),
        @artist_gids);
    my $random = Random->new('random:', $filter->filter($::ListPlay));
    my @IDs    = $random->Draw($nb, [@$::Queue, $::SongID])
